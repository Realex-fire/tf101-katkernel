...
#if defined(CONFIG_SUSPEND) && defined(CONFIG_CPU_CONTROL)
...
static void hp_resume(struct early_suspend *handler){
...
static void hp_suspend(struct early_suspend *handler) {
	if (!auto_hotplug_enabled || single_core_mode)
		return;
...
static struct early_suspend hp_suspend_handler = {
	.suspend = hp_suspend,
	.resume = hp_resume,
};
...
int __init auto_hotplug_init(void)
{
	...
	register_early_suspend(&hp_suspend_handler);
}
